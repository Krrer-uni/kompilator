...
std::vector<std::string> VALUE_BLOCK::translate_block() {
  ...
  switch (_value_type) {
    case basic_blocks_types::VAL_VAR:code = _var_block->translate_block();
      break;
    case basic_blocks_types::VAL_LIT:code = _num_block->translate_block();
      break;
...
std::string VAR_BLOCK::get_memory_adress() {
  ...
  if (_var_type == value_var) {
    msg = " " + std::to_string(_memory_block) + "   [ @" + _var_name + " ]";
  } else {
    msg = "I " + std::to_string(_memory_block) + "   [ @" + _var_name + " ]";
  }
  return msg;
}
